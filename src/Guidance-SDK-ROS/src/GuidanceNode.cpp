...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    {
        motion *g_motion = (motion*)content;
        if (verbosity > 1) {
            printf( "frame index: %d, stamp: %d\n", g_motion->frame_index, g_motion->time_stamp );
            printf( "px:%f py:%f pz:%f vx:%f vy:%f vz:%f\n", 
				g_motion->position_in_global_x, g_motion->position_in_global_y, g_motion->position_in_global_z, 
				g_motion->velocity_in_global_x, g_motion->velocity_in_global_y, g_motion->velocity_in_global_z);
			printf( "q:%f %f %f %f\n", g_motion->q0, g_motion->q1, g_motion->q2, g_motion->q3);
        }
		...
		odom_trans.transform.translation.x = g_motion->position_in_global_x;
		odom_trans.transform.translation.y = g_motion->position_in_global_y;
		odom_trans.transform.translation.z = g_motion->position_in_global_z;
		...
		//set the position
		odom.pose.pose.position.x = g_motion->position_in_global_x;
		odom.pose.pose.position.y = g_motion->position_in_global_y;
		odom.pose.pose.position.z = g_motion->position_in_global_z;
		odom.pose.pose.orientation = odom_quat;

		for (int i = 0; i < 3; i++)
		{
			odom.pose.covariance[i + 6 * i] = g_motion->uncertainty_location[i];
		}
		

		//set the velocity
		odom.child_frame_id = "base_link";
		odom.twist.twist.linear.x = g_motion->velocity_in_global_x;
		odom.twist.twist.linear.y = g_motion->velocity_in_global_y;
		odom.twist.twist.linear.z = g_motion->velocity_in_global_z;

		for (int i = 0; i < 3; i++)
		{
			odom.twist.covariance[i + 6 * i] = g_motion->uncertainty_velocity[i];
		}
...
int main(int argc, char** argv)
{
	...
	odom_pub			    = my_node.advertise<nav_msgs::Odometry>("/guidance/odom",1);
